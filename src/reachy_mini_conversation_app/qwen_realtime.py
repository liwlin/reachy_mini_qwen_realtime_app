...
def _openai_tool_specs_to_qwen(specs: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _resolve_qwen_voice(profile_voice: str) -> str:
    ...
    return voice_map.get(profile_voice.lower(), DEFAULT_VOICE_BY_BACKEND[QWEN_BACKEND])
...
class QwenRealtimeHandler(AsyncStreamHandler):
    """DashScope/Qwen Realtime API handler for fastrtc Stream."""
    ...
    def copy(self) -> "QwenRealtimeHandler":
        """Create a copy of the handler."""
        return QwenRealtimeHandler(self.deps, self.gradio_mode, self.instance_path)
    ...
    async def start_up(self) -> None:
        ...
        if self.gradio_mode and not qwen_api_key:
            await self.wait_for_args()  # type: ignore[no-untyped-call]
            args = list(self.latest_args)
            textbox_api_key = args[3] if len(args) > 3 and args[3] else None
            if textbox_api_key is not None:
                qwen_api_key = textbox_api_key
            else:
                qwen_api_key = config.QWEN_API_KEY
    ...
    async def _run_realtime_session(self, qwen_api_key: str) -> None:
        ...
        async with self._connect(url, headers) as websocket:
            ...
            await self._send_session_update()
            logger.info(
                "Qwen realtime session initialized with model=%s voice=%s", config.MODEL_NAME, self.get_current_voice()
            )
        ...
        if event_type == "response.audio.delta":
            ...
            if isinstance(audio_delta, str) and audio_delta:
                if self.gradio_mode and self.deps.head_wobbler is not None:
                    self.deps.head_wobbler.feed(audio_delta)
    ...
    async def _handle_tool_result(self, bg_tool: ToolNotification) -> None:
        ...
        if bg_tool.tool_name == "camera" and image_b64 is not None:
            np_img = self.deps.camera_worker.get_latest_frame() if self.deps.camera_worker is not None else None
            if np_img is not None:
                await self.output_queue.put(
                    AdditionalOutputs({"role": "assistant", "content": gr.Image(value=np_img)})
                )
    ...
    async def emit(self) -> Tuple[int, NDArray[np.int16]] | AdditionalOutputs | None:
        """Emit audio/text output for the speaker or UI."""
