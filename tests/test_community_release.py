...
def test_local_control_release_contract() -> None:
    ...
    assert (static_root / "media.html").is_file()
    assert (static_root / "setup.html").is_file()
    assert (static_root / "app.js").is_file()
    assert (static_root / "local-webrtc.js").is_file()
...
def test_community_project_metadata_and_entry_points() -> None:
    ...
    assert metadata["project"]["version"] == "1.0.1+qwen.5"
    assert (project_root / "docs" / "release-notes-v1.0.1-qwen.5.md").is_file()
